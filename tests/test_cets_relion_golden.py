...
def test_c6_relion_chain_matches_matrix_model(image_n, vol):
    ...
    entries = [
        RelionAlignmentEntry(
            z_index=i, nominal_stage_tilt_angle=-30.0 + 10 * i, x_tilt=0.4, y_tilt=-30.0 + 10 * i + 0.1,
            z_rot=-95.0 + 0.02 * i, x_shift_angst=float(rng.normal(0, 40)), y_shift_angst=float(rng.normal(0, 40)),
            pre_exposure=3.0 * i,
        )
        for i in range(7)
    ]
    rel = RelionAlignment(tomo_name="TS", pixel_size_a=s, volume_size_px=vol, hand=-1, voltage=300,
                          spherical_aberration=2.7, amplitude_contrast=0.07, entries=entries)
    ...
    model = relion_ts.RelionTomogramModel(
        ...
        tomo_dims_px=vol, image_dims_px=image_n, pixel_size_a=s,
    )
    ...
    cets = alignment_to_cets(hub, tilt_series_id="TS", alignment_name="relion", image=img, reference=ref,
