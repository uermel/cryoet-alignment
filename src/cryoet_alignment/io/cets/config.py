...
class Resolver:
    ...
    def optional(
        ...
        *,
        ...
        if (
            option in self.cli
            or (self.config and self.config.lookup(option, self.package, self.command, self.series) is not _MISSING)
            or (companion is not _MISSING and companion is not None)
            or (discovered is not _MISSING and discovered is not None)
        ):
